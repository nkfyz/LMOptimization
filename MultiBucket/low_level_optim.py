...
class LowLevelZeroOptimizer(OptimizerWrapper):
    ...
    def __init__(
        ...
        if not OPEN_MULTI_BUCKET_STORE:
            self._bucket_store = BucketStore(self.dp_pg)
        else:
            self._bucket_stores = [BucketStore(self.dp_pg) for _ in range(len(self.optim.param_groups))]
    ...
    def _run_reduction_parallel(self):
        
        # Warning: current implementation is not parallel as parallel reduction may cause deadlock.
        # (Yaozheng Fang) TODO: implement parallel reduction
        for _bucket_store in self._bucket_stores:
            self._bucket_store = _bucket_store
            self._run_reduction()

    def _add_to_bucket(self, param, group_id):
        param_size = param.numel()
        
        if OPEN_MULTI_BUCKET_STORE:
            self._bucket_store = self._bucket_stores[group_id]
    ...
    def backward(self, loss, retain_graph=False):
        ...
        # self._partition_grads is False
    ...
    def _reduce_grad(self, partition_grad):
        ...
        # self._overlap_communication True
